...
class _AddEditCategoryScreenState extends State<AddEditCategoryScreen> {
  ...
  Future<void> _save() async {
    ...
    if (_isEditing) {
      success = await prov.updateCategory(widget.category!.copyWith(name: _nameController.text.trim(), iconName: _selectedIcon));
    } else {
      success = await prov.addCategory(userId: userId, name: _nameController.text.trim(), type: _type, iconName: _selectedIcon);
    }
    if (!mounted) return;
  ...
  Widget _typeChip(String label, String value) {
    ...
    return GestureDetector(
      onTap: () => setState(() { _type = value; _selectedIcon = value == 'expense' ? 'food' : 'salary'; }),
      child: Container(
