...
class _LoanTrackingScreenState extends State<LoanTrackingScreen>
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            ...
            if (currentUserId != 0) {
              context.read<LoanProvider>().loadLoans(currentUserId);
            }
          });
        },
  ...
  Widget _buildTabContent({
    ...
    final isEmpty = activeLoans.isEmpty && paidLoans.isEmpty;
  ...
  Future<void> _showPaymentDialog({
    ...
    final ok = await context.read<LoanProvider>().recordPayment(
      loanId,
      confirmedAmount,
      userId,
    );
  ...
  Future<void> _showDeleteDialog(BuildContext context, Loan loan) async {
    ...
    if (loanId == null || userId == 0) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Không thể xóa khoản vay')));
      return;
    }
