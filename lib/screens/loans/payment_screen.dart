...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (success) {
      ...
      final message = context.read<LoanProvider>().errorMessage ?? 'Không thể ghi nhận thanh toán';
  ...
  Widget build(BuildContext context) {
    ...
    final color = widget.loan.type == 'borrow' ? AppTheme.tertiary : AppTheme.loanColor;

    return Scaffold(
      backgroundColor: AppTheme.surface,
      appBar: AppBar(
        title: Text(widget.loan.type == 'borrow' ? 'Thanh toán khoản vay' : 'Ghi nhận thu nợ'),
        ...
        child: Form(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Container(
              ...
              decoration: BoxDecoration(color: color.withAlpha(16), borderRadius: BorderRadius.circular(AppTheme.radiusLg)),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(widget.loan.personName, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700, color: color)),
                const SizedBox(height: 6),
                Text('Còn lại: ${Formatters.currency(widget.loan.remainingAmount)}', style: Theme.of(context).textTheme.bodyMedium),
              ]),
            ),
            ...
            TextFormField(
              ...
              validator: (value) {
                ...
                if (amount > widget.loan.remainingAmount) return 'Số tiền không được vượt quá dư nợ còn lại';
                return null;
              },
            ),
            const SizedBox(height: 20),
            ...
            SizedBox(height: 52, child: ElevatedButton(onPressed: _submit, child: const Text('Lưu thanh toán'))),
          ]),
        ),
      ),
    );
  ...
  Widget _label(String text) => Text(text, style: Theme.of(context).textTheme.labelMedium?.copyWith(fontWeight: FontWeight.w600, letterSpacing: 0.8));
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Text(Formatters.date(_paymentDate), style: const TextStyle(color: AppTheme.onSurface)),
        ]),
      ),
    );
  }
