...
void main() {
  ...
  if (!Platform.isAndroid) {
    ...
    testWidgets('Luồng tích hợp vay và cho vay chỉ chạy trên Android', (
      tester,
    ) async {
      markTestSkipped(
        'Chỉ chạy trên Android: Genymotion hoặc điện thoại thật.',
      );
    });
    return;
  }
  ...
  testWidgets(
    'Luồng vay qua danh sách và chi tiết, trả nợ và cho vay dùng dữ liệu biệt lập',
    (tester) async {
      ...
      await runDebtStep(tester, 'tích hợp vay: nhấn lưu', () async {
      ...
      await runDebtStep(tester, 'tích hợp trả nợ: nhấn lưu', () async {
      ...
      await runDebtStep(tester, 'tích hợp cho vay: nhấn lưu', () async {
      ...
      final txns = await runDebtValue(
        ...
        () => fixture.db.query('transactions', orderBy: 'id ASC'),
      );
      expect(
        txns.map((txn) => txn['type']),
        containsAll(['loan', 'expense', 'lend']),
      );
  ...
  testWidgets(
    'Luồng sửa và trả hết khoản vay cập nhật chi tiết và trạng thái',
    (tester) async {
      ...
      await runDebtStep(tester, 'tích hợp sửa: lưu thay đổi', () async {
      ...
      await runDebtStep(tester, 'tích hợp trả hết: lưu thanh toán', () async {
      ...
      await pumpDebtUntil(
        tester,
        () => find.text('Đã trả').evaluate().isNotEmpty,
      );
  ...
  testWidgets('Luồng cho vay thu nợ và xóa hoàn tác dữ liệu qua giao diện', (
    ...
    await runDebtValue(
      ...
      () =>
          _themKhoanDebt(fixture, type: 'lend', personName: 'Bob', amount: 300),
    );
    ...
    expect(
      await runDebtValue(
        tester,
        'tích hợp xóa: đọc số dư hoàn tác',
        fixture.accountBalance,
      ),
      1000,
    );
  ...
  testWidgets(
    'Luồng đổi loại vay cho vay và tài khoản liên kết cập nhật dòng tiền',
    (tester) async {
      final fixture = await _taoFixtureTichHop(tester);
      addTearDown(fixture.dispose);
      ...
      await runDebtStep(tester, 'tích hợp đổi loại: lưu thay đổi', () async {
        editButton.onPressed!();
        await waitForDebtCondition(
      ...
      expect(
        await runDebtValue(
          tester,
          'tích hợp đổi loại: số dư tiền mặt',
          fixture.accountBalance,
        ),
        1000,
      );
  ...
  testWidgets(
    'Luồng validation giao diện từ chối tạo khoản vay khi thiếu tài khoản',
    (tester) async {
  ...
  testWidgets('Luồng validation giao diện từ chối thanh toán vượt dư nợ', (
    ...
    expect(
      find.text('Số tiền không được vượt quá dư nợ còn lại'),
      findsOneWidget,
    );
    expect(
      await runDebtValue(
        tester,
        'validation thanh toán: đọc số lần trả',
...
Future<void> _dungDeQuanSat(WidgetTester tester, String trangThai) async {
  ...
  await tester.runAsync(() async => Future<void>.delayed(_thoiGianQuanSat));
}
